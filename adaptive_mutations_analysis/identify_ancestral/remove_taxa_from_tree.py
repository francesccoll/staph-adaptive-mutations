...
def parse_arguments():
    description = "Script to remove taxa from a phylogenetic tree in Newick format"
    parser = argparse.ArgumentParser(description=description)

    parser.add_argument(
        "-i", "--input_tree", action="store", dest="input_tree",
        help="Input phylogenetic tree file in Newick format",
        required=True, default=True, metavar="INPUT_TREE")
    parser.add_argument(
        "-t", "--taxa_to_remove", action="store", dest="taxa_to_remove",
        help="File with taxa ids to remove. One taxon id per line expected.",
        required=True, default=True, metavar="IDS_TABLE")
    parser.add_argument(
        "-o", "--output_tree", action="store", dest="output_tree",
        help="Output phylogenetic tree file in Newick format with taxa removed",
        required=True, default=True, metavar="OUTPUT_TREE")
...
def _main():
    ...
    input_files = [args.input_tree, args.taxa_to_remove]
    for input_file in input_files:
        if not os.path.exists(input_file):
            logging.error(f'Input file {input_file} not found!')
            sys.exit(-1)

    # Extracting taxa ids to remove
    taxa_to_remove = dict()  # dictionary to save taxa ids
    if args.taxa_to_remove is not None:
        logging.info(f"Opening input file {args.taxa_to_remove}")
        with open(args.taxa_to_remove, 'r') as taxa_to_remove_file:
            for taxa_to_remove_line in taxa_to_remove_file:
                    taxa_id = taxa_to_remove_line.strip()
                    taxa_to_remove[taxa_id] = '-'
    ...
    # Removing taxa
    for taxon_to_remove in taxa_to_remove.keys():
        if taxon_to_remove in tree_taxa:
            # Note: the argument of prune_taxa_with_labels must be a list for it to work, so
            #       square brackets must be used
            tree.prune_taxa_with_labels([taxon_to_remove])
            logging.info(f'Removed {taxon_to_remove} from input tree')
        else:
            logging.warning(f'Taxon {taxon_to_remove} not in input tree so it cannot be removed!')

    # Printing basic information on tree after removing taxa
    logging.info(f"Output tree information (after removing taxa)")
    print('Number of taxa in tree: ' + str(len(tree.taxon_namespace)))
    print('Number of leaf nodes in tree: ' + str(len(tree.leaf_nodes())))
    print('Number of internal nodes in tree: ' + str(len(tree.internal_nodes())))
