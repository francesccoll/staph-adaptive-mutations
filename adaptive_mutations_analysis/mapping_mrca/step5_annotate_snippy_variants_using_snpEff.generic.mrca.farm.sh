...
do
	...
	if [ ! -f $ann_vcf ]
	then
		bsub -q normal -J $pair"_ann" -o $pair"_ann.out" -R "select[mem > 5000] rusage[mem=5000]" -M 5000 "java -jar $wd'snpEff.jar' ann -nodownload -no-downstream -no-upstream -verbose -stats $annotation_stats_file -c $config_filename $genome $vcf_file > $ann_vcf"
	fi
done